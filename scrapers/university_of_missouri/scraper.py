...
async def scrape_page(page):
    ...
    bio_containers = await page.query_selector_all('.archive-person')

    for container in bio_containers:
        # Extract name
        name_element = await container.query_selector('.person-name')
        name = await name_element.inner_text() if name_element else None

        # Extract URL
        link_element = await container.query_selector('.person-link')
        url = await link_element.get_attribute('href') if link_element else None

        # Extract position
        position_element = await container.query_selector('.person-job-title')
...
async def advance_page(page):
    ...
    next_button = await page.query_selector('.next.page-numbers')
    if next_button:
